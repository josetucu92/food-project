...
export const getApiRecipes = async () => {
  const apiKeys = apiKey || apiKey2 || apiKey3 || apiKey4 || apiKey5;
  try {
    const apiRecipesPromise = await axios.get(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${apiKeys}&addRecipeInformation=true&number=${amountRecipes}`
    );
    const apiRecipes = apiRecipesPromise.data?.results.map((el) => {
      return {
        ...
        steps:
          ...
          el.analyzedInstructions[0]?.steps?.map((item) => item.step),
      };
    });
    //console.log(apiRecipes[0].steps)
    return apiRecipes;
  } catch (error) {
